...
def test_deserialize_struct():
    ...
    request = _serialize.EncapsulatedRequest(
        seq=1,
        rpc='test',
        data=b'\x82\xa4name\xa4test\xa5value\x05'
    )
...
def test_serialize_struct():
    ...
    assert _serialize.serialize_message(struct, ExampleStruct) == b'\x82\xa4name\xa4test\xa5value\x05'


def test_serialize_dict():
    """Test serialization of dictionaries"""

    assert _serialize.serialize_message({
        'name': 'test',
        'value': 5
    }, ExampleStruct) == b'\x82\xa4name\xa4test\xa5value\x05'

    # Test dict serialization using generic dict type
    assert _serialize.serialize_message({
        'name': 'test',
        'value': 5
    }, Dict[str, Union[str, int]]) == b'\x82\xa4name\xa4test\xa5value\x05'
