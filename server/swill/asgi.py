...
class AsgiApplication:
    def __init__(
        self,
        paths: t.List[str],
        lifecycle_handler: t.Callable,
        request_handler: t.Callable,
    ):
        self.paths = paths
            ...
            if request_event["type"] == "websocket.connect":
                ...
                try:
                    ...
                    if e.code < 1000:
                        ...
                        connection_data.http_response.status_code = (
                            403  # Default return code
                        )
                    ...
                    await send(
                        {"type": "http.response.body", "body": str(e).encode("utf-8")}
                    )
                    return False
                await send(
                    {
            ...
            if receive_task in done:
                ...
                if event["type"] == "websocket.receive":
                    ...
                    handler_task = asyncio.create_task(
                        self.request_handler(event["bytes"], connection), name="func"
                    )
